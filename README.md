...
1. The deepCT prediction files must be reassmembled into a single
text document. This is easily accomplished with Linux's `cat` command:
```bash
cat \
test_results_000.tsv \
test_results_001.tsv \
...
test_results_031.tsv \
test_results_032.tsv \
> test_results.tsv
```

The output predictions file contains one row of predicted term weights
for each document passage.

2. In addition to the predictions file, HDCT requires a dataset file that
has the same number of rows as the output predictions file, and that
identifies the document ID and passage number of each row in the predictions
...
------------------|------------------|-------------|------------
Baseline HDCT MSMARCO Doc | - | 0.20889 | 0.0
t5 body only | 2 | 0.267145665 | 0.05825
tanh passage normalization | 3| 0.24821 | 0.039321
